...
function useNumberFlowElementReady(): boolean {
  // Always starts false to match SSR (no `customElements` on the server),
  // and only flips inside an effect - reading `customElements` in the
  // useState initializer would return true on the client's very first
  // render (the custom element registers at import time), mismatching the
  // server-rendered static text and triggering a hydration error.
